<div>
    <div class="mb-12 flex justify-between items-end">
        <div>
            ...
            <p class="text-[14px] text-[#555555] mt-1">Track your requests. The progress bar under status shows how far along each ticket is.</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-[#f0f0f0]">
                <thead class="bg-[#f7f7f7]">
                    <tr>
                        <th class="px-6 py-3 text-left text-[11px] font-medium text-[#999999] uppercase tracking-widest">No.</th>
                        <th class="px-6 py-3 text-left text-[11px] font-medium text-[#999999] uppercase tracking-widest">Service</th>
                        <th class="px-6 py-3 text-left text-[11px] font-medium text-[#999999] uppercase tracking-widest">Priority</th>
                        ...
                        <th class="px-6 py-3 text-left text-[11px] font-medium text-[#999999] uppercase tracking-widest">Due Date</th>
                        <th class="px-6 py-3 text-right text-[11px] font-medium text-[#999999] uppercase tracking-widest">Action</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-[#f0f0f0]">
                    @forelse($tickets as $ticket)
                        <tr class="hover:bg-[#fafafa] transition-colors">
                            ...
                            <td class="px-6 py-5 whitespace-nowrap text-[13px] text-[#999999]">
                                {{ $ticket->due_date->format('M d, g:i A') }}
                            </td>
                            <td class="px-6 py-5 whitespace-nowrap text-right">
                                <div class="flex items-center justify-end gap-4">
                                    @if($ticket->recommendation && $ticket->recommendation->file_path)
                                        <a href="{{ Storage::url($ticket->recommendation->file_path) }}" target="_blank" class="text-[12px] font-bold text-[#2d2d2d] underline flex items-center gap-1">
